...
const socketForPort8081 = require('socket.io-client')('http://localhost:8081');
        ...
        Online.findOne({ userId: socketId }, (err, data) => {
            ...
            if (data !== null) {
                ...
                User.updateOne({ userName: username }, { login: false }, (err, info) => {
                    ...
                    io.emit("disconnectOn8080", username);
                })
...
socketForPort8081.on('loginOn8081', socket => {
...
socketForPort8081.on('messagefrom8081', socket => {
...
socketForPort8081.on('disconnectOn8081', socket => {
